...
func validate_ecdsa_secp256k1{range_check_ptr}(sv : SignatureVerification):
...
@view
func test_p2pkh_with_compressed_pubkey02{range_check_ptr}():
    let (sv : SignatureVerification) = test_utils.load_p2pkh_tx_from_json(
        './resources/tx/p2pkh02.json'
    )
...
@view
func test_p2pkh_with_compressed_pubkey03{range_check_ptr}():
    let (sv : SignatureVerification) = test_utils.load_p2pkh_tx_from_json(
        './resources/tx/p2pkh03.json'
    )
    validate_ecdsa_secp256k1(sv)
    return ()
end
...
# uncompressed pubkey
@view
func test_p2pkh_with_uncompressed_pubkey04{range_check_ptr}():
    let (sv : SignatureVerification) = test_utils.load_p2pkh_tx_from_json(
        './resources/tx/p2pkh04.json'
    )
    validate_ecdsa_secp256k1(sv)
    return ()
end
